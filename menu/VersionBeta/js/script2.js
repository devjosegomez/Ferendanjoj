...
var getJsonNew = new XMLHttpRequest();
...
function getJsonLenght() {
    getJsonNew.open('GET', 'http://localhost:3000/db/', false);
    getJsonNew.onreadystatechange = function () {
        if (getJsonNew.readyState == XMLHttpRequest.DONE) {
            JsonDB = JSON.parse(getJsonNew.responseText);
        }
    }
    getJsonNew.send();
...
function addTab(_id) {
    ...
    var content = getContent(_id);
    var weekDay = today.getDay();
    var tabColor = getColor(_id);
